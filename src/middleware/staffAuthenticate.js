...
const staffAuthorize = (req, res, next) => {
    ...
    if (getAccessToken && getRefreshToken) {
        jwt.verify(getAccessToken, process.env.SCKEY_STAFF, (accessToken_err, accessToken_decoded) => {
            if (accessToken_err) {
                // Access Token expired → check Refresh Token
                jwt.verify(getRefreshToken, process.env.SCKEY_STAFF, (refreshToken_err, refreshToken_decoded) => {
                    if (!refreshToken_err) {
